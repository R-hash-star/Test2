##tetracene層内計算
import os
os.environ['HOME'] ='/home/asai'
import pandas as pd
import time
import sys
from tqdm import tqdm
sys.path.append(os.path.join(os.environ['HOME'],'Working/interaction/'))
from make3 import exec_gjf##計算した点のxyzfileを出す
...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        init_para_list = []
        A1 = 0; A2 = 0
        theta_list =[24.0,24.5,25.0,25.5,26.0,26.5,27.0]##全体を掃く＋細かいところ
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','E','E_p','E_t','machine_type','status','file_name'])
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=2:##get Eの長さは計算した分子の数
            ...
            Et=float(E_list[0]);Ep=float(E_list[1])##6分子に向けてep作成　ep1:b ep2:a
            E = 4*Et+2*Ep##エネルギーの値も変える
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.DataFrame([{**params_dict,'E':0.,'E_p':0.,'E_t':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name}])
